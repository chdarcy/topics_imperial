...
rotations and hence large weight jumps (Nadler,~2008).
% =====================================================================
...
\multirow{5}{*}{spot 1Y--30Y}
  ...
  & Vol-Scaled   & 0.42 & 67   & 84  & 111 & 53.5 & $-$0.03 \\
...
\multirow{5}{*}{spot 1Y--10Y}
  ...
  & Vol-Scaled   & 0.41 & 98   & 126 & 87  & 52.3 & 6.65 \\
...
\multirow{5}{*}{adjacent forwards}
  ...
  & Vol-Scaled   & 0.62 & 116  & 98  & \textbf{26}  & 49.2 & 7.98 \\
...
\begin{itemize}
  \item The z-score MR signal improves the Sharpe ratio on two of three
        ...
        (0.34~$\to$~1.44).  On spot~1Y--10Y the static butterfly
        already performs strongly (0.84) and MR provides no uplift
        (0.77).
  \item Carry+MR is the strongest variant on spot~1Y--30Y (1.32) and
      adjacent~forwards (1.42), matching or narrowly exceeding MR alone while
        achieving the tightest max drawdown on 1Y--30Y (77\,bps).  It
        underperforms on spot~1Y--10Y (0.41), where the static strategy
  ...
  \item Vol-scaling underperforms the static baseline on two of three
        curves, only improving performance on the forward curve where
        the baseline was weakest, consistent with vol-scaling acting
        as risk management rather than signal enhancement.
...
variant-to-variant Sharpe dispersion ($-$0.42 to 1.44).  The carry+MR
overlay achieves a Sharpe of 1.42, essentially matching the z-score MR
strategy (1.44) while reducing annualised volatility from 191 to
166\,bps.  Both PnL (545~$\to$~467\,bps) and volatility fall roughly
...
Volatility-scaling (116\,bps cumulative, Sharpe 0.62) sits
below the static in cumulative PnL terms; it reduces drawdown
substantially (112~$\to$~26\,bps) but at the cost of reduced absolute
returns, consistent with the vol floor preventing over-leverage during
periods of high edge.  Momentum produces deep and sustained drawdowns.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{strategy_performance_fwd.png}
...
``false'' mean-reversion trades where the butterfly is cheap but lacks
supportive roll-down (e.g.\ during structural curvature flattening in
the cutting regime).  This is consistent with Dreher et al.'s~(2020)
...
forward curve (Sharpe 0.34~$\to$~0.62) by normalising risk across
regimes.  However, it underperforms on spot curves.  Two mechanisms
likely explain this.  First, the 63-day lookback adapts slowly to regime
changes, so lagged volatility estimates can increase exposure just as
large shocks occur (e.g.\ higher MaxDD on spot~1Y--10Y).  Second, if
curvature dislocations are concentrated in high-volatility periods,
vol-scaling reduces position size precisely when expected returns are
highest, dampening both PnL and Sharpe.

\textbf{Momentum} shows strong curve dependence.  It performs well on
spot~1Y--30Y (Sharpe 1.18, 431\,bps) but reverses on spot~1Y--10Y
($-$0.86) and adjacent~forwards ($-$0.42).  The spot~1Y--10Y result appears
event-driven rather than structural: a near-50\% hit rate combined with
extreme negative skew ($-$11.92) suggests losses are concentrated in a
small number of large shocks, most likely the September~2024 BoE rate
cut.  The outperformance on spot~1Y--30Y may reflect persistent
long-end flows (e.g.\ LDI hedging), but with only a single cycle in the
sample this remains speculative.  Overall, the results are
...
\begin{enumerate}
  ...
  \item \textbf{Carry+MR overlay is the most consistently robust variant.}
        By conditioning entries on carry agreement, it achieves Sharpe
        ratios of 1.32 and 1.42 on two of three curves.  On the forward
        curve it matches z-score MR risk-adjusted returns (1.42 vs.\
        1.44) while reducing volatility, acting primarily as a bet-size
        filter rather than a loss-avoidance mechanism.
...
These results demonstrate that forward-rate curvature strategies,
implemented with dynamic PCA and structural PC identification, provide
a robust framework for isolating and trading term-structure curvature.
However, performance is driven primarily by the addition of signal
overlays, particularly carry-augmented mean reversion, rather than the
curvature exposure itself.  This reinforces the interpretation of the
PCA butterfly as a construction for expressing relative-value views,
rather than a standalone trading strategy.
