...
curve.  We further test five literature-inspired strategy
variants---volatility-scaling, momentum, carry overlay, PC-score
momentum, and regime filtering---and find that the carry+mean-reversion
overlay is the most robust enhancement (Sharpe 1.32--1.42 on two of
three curves).  PC-score momentum fails uniformly, confirming that GBP
curvature is mean-reverting rather than trending in this sample.
...
(iii)~we evaluate five literature-inspired signal and sizing variants,
...
We test five additional strategies motivated by the academic literature
...
\subsubsection{PC3-Score Momentum}

Following Oprea~(2022) and the Credit Suisse ``PCA Unleashed''
framework, we use the trailing 10-day average PC3 (curvature) score
as a directional signal.  Positive trailing score $\to$ long butterfly;
negative $\to$ short.  This approach uses the factor itself rather
than the spread level as the signal source.

\subsubsection{Regime Filter}

Lammi~(2022) and Hariparsad and Mar\'{e}~(2024) highlight how
monetary-policy regimes affect yield-curve arbitrage returns.  Our regime filter trades
only when trailing 63-day PnL volatility exceeds its expanding 50th
percentile, shutting off the strategy during the low-volatility hold
regime when curvature risk premia are insufficient to justify trading.
...
mean-reversion signal, and five strategy variants.
...
\multirow{7}{*}{spot 1Y--30Y}
  ...
  & PC3 Mom.     & $-$0.35 & $-$128 & 188 & 291 & 50.1 & $-$0.49 \\
  & Regime Filt. & 0.15 & 53   & 174 & 175 & 34.3 & 0.46 \\
\midrule
\multirow{7}{*}{spot 1Y--10Y}
  ...
  & PC3 Mom.     & $-$1.30 & $-$343 & 134 & 414 & 47.3 & $-$12.14 \\
  & Regime Filt. & \textbf{0.89} & \textbf{232} & 132 & 46  & 25.3 & 12.86 \\
\midrule
\multirow{7}{*}{fwd 1Y}
  ...
  & PC3 Mom.     & $-$3.83 & $-$2175 & 289 & 2194 & 41.4 & $-$2.07 \\
  & Regime Filt. & 0.29 & 155  & 269 & 112 & 21.8 & 1.37 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item PC3-score momentum fails uniformly (Sharpe $-$0.35 to $-$3.83),
        confirming that GBP curvature in this sample is mean-reverting,
        not trending.
...
steadily, while momentum and PC3-score momentum produce deep and
sustained drawdowns.
...
\subsection{Sharpe Ratio Comparison}

Figure~\ref{fig:sharpe_bar} provides a visual comparison of Sharpe
ratios across all strategies and curves.  The clustering of
mean-reversion-based strategies (z-score MR, carry+MR) at the top
and momentum-based strategies at the bottom is consistent across
configurations, strongly supporting the mean-reverting nature of GBP
curvature.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{variant_sharpe_comparison.png}
\caption{Sharpe ratio comparison across all strategy variants and curve
configurations.  Negative Sharpe ratios indicate momentum and PC3-score
strategies that lose money in a mean-reverting curvature environment.}
...
\subsubsection{Loading Shapes Over Time}

Figure~\ref{fig:loading_shapes} displays the PCA loading shapes at
three time snapshots (start, middle, end of the out-of-sample period)
for the spot 1Y--30Y strategy.  The curvature loading (green) maintains
its characteristic humped shape throughout, but the amplitude and
peak location shift across regimes.  The slope loading (red) steepens
during the cutting regime as easing expectations differentially affect
short and long maturities.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{loading_shapes_spot_1y-30y.png}
\caption{PCA loading shapes at three time points for the spot 1Y--30Y
strategy.  The curvature loading shape is qualitatively stable but
varies in amplitude.}
\label{fig:loading_shapes}
\end{figure}

\subsection{Regime Mismatch}

Figure~\ref{fig:regime_mismatch} shows the mismatch between
estimation-window and forward-window eigenvalue sums.  With a 252-day
rolling estimation window, weights calibrated during the hold regime
(low volatility) are applied to the cutting regime (high volatility)
during autumn~2024.  The forward eigenvalues spike relative to the
estimation eigenvalues, indicating that the estimated factor structure
underestimates the true variability of the market---particularly for
the level factor, which gains the most variance during cutting.

This mismatch has practical consequences:  weights calibrated from
low-volatility covariance matrices produce positions that are too large
relative to the actual risk being taken, leading to outsized PnL swings
during the initial cutting phase.  Volatility scaling (Section~5.3.1)
partially addresses this issue.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{regime_mismatch.png}
\caption{Regime mismatch: total eigenvalue sum from estimation window
(blue) versus 252-day forward window (red) on spot rates.  The
divergence during 2024~Q4 reflects the hold-to-cut transition.}
\label{fig:regime_mismatch}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{strategy_performance_spot_1y-30y.png}
\caption{Detailed strategy performance for the spot 1Y--30Y butterfly.
The static position accumulates PnL steadily but with larger drawdowns
than the signal-based version.}
\label{fig:perf_spot30}
\end{figure}
...
The five variants test different aspects of curvature factor dynamics:
...
\textbf{PC3-score momentum} fails uniformly (Sharpe $-$0.35 to
$-$3.83).  The curvature factor score is a contemporaneous measure of
curvature position; using its trailing average as a directional signal
amounts to ``buying high and selling low'' in a mean-reverting
environment.  This strongly confirms that GBP curvature is
mean-reverting, not trending, over the 10-day lookback horizon.

\textbf{Regime filter} provides modest improvement on spot~1Y--10Y
(0.84~$\to$~0.89) by concentrating trades in high-volatility periods
when curvature risk premia are larger.  However, the improvement is
small because the filter is relatively coarse (50th percentile
threshold) and misses the early stages of regime transitions.
...
For institutional implementation, our results suggest:
\begin{enumerate}
  \item Use forward-rate PCA for curvature butterfly construction.
  \item Identify PCs by loading shape, not eigenvalue rank.
  \item Combine z-score mean-reversion entry with a carry filter to
        avoid false positives.
  \item Re-estimate PCA at least daily with a rolling window of
        approximately one year.
  \item Volatility-scale positions when the underlying strategy exhibits
        high unconditional volatility dispersion across regimes.
\end{enumerate}
...
configurations, two base trading rules, and five literature-inspired
...
\begin{enumerate}
  ...
  \item \textbf{Carry+MR overlay is the most robust enhancement.}  By
        ...
        three curves.  PC-score momentum fails uniformly, confirming
        the mean-reverting nature of GBP curvature.
